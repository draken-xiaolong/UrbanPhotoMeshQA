...
- 本地原始数据、攻击数据与大缓存的唯一根目录是 `/Volumes/SANDISK-ELE/UrbanPhotoMeshQA-Data`。任何涉及数据或大文件同步的工作开始前必须先检查 `/Volumes/SANDISK-ELE` 已挂载；若未挂载，先提醒用户插入移动硬盘并暂停数据操作，禁止改用内置盘或创建同名替代目录。
