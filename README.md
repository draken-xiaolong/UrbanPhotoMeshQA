...
- 质检剔除3条与中度文件完全相同的QEM重度样本；正式清单为3493条：
  Train 1518、Val 760、Test 607、Blind 608，同一建筑不跨划分；
- 主模型是冻结 Point、Mesh、Morphology、Texture Base 上的四分支 MLP Quality Head；
- 仅用 Val 选模，Test/Blind 锁定；
- OQI SRCC：Val `0.521`、Test `0.557`、Blind `0.444`；
...
Blind 泛化的第一阶段 GPU 实验见
[GENERALIZATION_EXPERIMENT_RUNBOOK_ZH.md](docs/GENERALIZATION_EXPERIMENT_RUNBOOK_ZH.md)。
